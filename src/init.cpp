...
std::string HelpMessage(HelpMessageMode hmm)
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: Woochain.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: Woochaind.pid)") + "\n" +
    ...
    if (hmm == HMM_BITCOIND)
    {
        ...
        "\n" + _("SSL options: (see the Woochain Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // ppcoin: -mintxfee and -minrelaytxfee options of Woochain disabled
    ...
    // Make sure only a single Woochain process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Woochain is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Woochain version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Woochain server starting\n");
    ...
    // requested to kill Woochain-qt during the last operation. If so, exit.
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Woochain") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Woochain to complete") << "\n";
    ...
    GenerateWoochains(GetBoolArg("-gen", false), pwalletMain);
