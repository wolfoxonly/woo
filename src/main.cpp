...
// Copyright (c) 2011-2015 The Woochain developers
...
const string strMessageMagic = "Woochain4.0 Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("Woochain-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in Woochain
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Woochain", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 10/4/2018 WOO genesis";// "Matonis 10-APR-2018 Parallel Currencies And The Roadmap To Monetary Freedom";
        CTransaction txNew;
        txNew.nTime = 1523348479;//
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523348479;// 1345084287;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
      //  block.nNonce   = 2179302059u;
        block.nNonce = ByteReverse(7503565);//<zxb>
        if (fTestNet)
        {
...
/////////////////////////////////zxb11111111111111111111111
 ...
 printf("block.nNonce 11111111111111 zxb:%d \n" ,block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("0xc0db796be78ae0d39ffc3b924bea566c9d221c7bc47a972d779f575310549920"));
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          printf("POW pindexPrev.nHeight===%d\n", pindexPrev->nHeight);
           ...
           {
                pblock->vtx[0].vout[0].nValue = 500000000 * COIN;//首块产量<zxb>
           }
           else
           {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("WoochainMiner : proof-of-work not meeting target");

    //// debug print
    printf("WoochainMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WoochainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WoochainMiner : ProcessBlock, block not accepted");
    }
...
#ifdef TESTING
void WoochainMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void WoochainMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    try { loop {
        ...
        printf("Running WoochainMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("WoochainMiner terminated\n");
        throw;
    }
}

void GenerateWoochains(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&WoochainMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&WoochainMiner, pwallet, false));
#endif
}
