...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Woochain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Woochain
    // Mac: ~/Library/Application Support/Woochain
    // Unix: ~/.Woochain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Woochain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Woochain.conf"));
    ...
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        ...
        else
        {
            ...
            try {
                const char* content = "rpcuser=Woochainrpc\nrpcpassword=Woochainrpcpassword";
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Woochain.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override Woochain.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Woochaind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Woochain will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.Woochain.it/wiki/BIP_0014)
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Woochain:" << FormatVersion(WOOCHAIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
