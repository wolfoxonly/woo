...
The RPM spec file provided here is for Woochain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Woochain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Woochain git repository](https://github.com/Woochain/Woochain) but are not part of
...
Currently only `Source10` is used, for the example `Woochain.conf` file.
...
in the Woochain git repository. At present only an SVG version of the Woochain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Woochain
...
needed when 0.12.1 is released, a proper fix is already in the Woochain git
master branch.
...
idea to build Woochain Core against a static version of that library compiled
...
The `Woochain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Woochain-server` package. At this point, most RPM based
...
The `Woochain-server` package should have SELinux support. How to properly do
...
Woochain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Woochain/). He has been
packaging Woochain for Fedora at least since 2012.
...
4. I renamed the `Woochain` package that contains the Qt GUI to `Woochain-core` as
that appears to be how the general population refers to it, in contrast to
`Woochain-xt` or `Woochain-classic`. I wanted to make sure the general population
