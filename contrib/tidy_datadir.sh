#!/bin/bash
# Copyright (c) 2013 The Woochain Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Woochain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Woochain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Woochain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Woochain 0.7 datadir."
    ;;
  3)
    echo "Detected Woochain pre-0.8 datadir."
    ;;
  4)
    echo "Detected Woochain 0.8 datadir."
    ;;
esac
