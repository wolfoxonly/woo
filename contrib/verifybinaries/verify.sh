...
###   Woochaincore.org and Woochain.org and compares them.
...
WORKINGDIR="/tmp/Woochain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://Woochaincore.org"
HOST2="https://Woochain.org"
BASEDIR="/bin/"
VERSIONPREFIX="Woochain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'Woochain-',
...
if [ $? -ne 0 ]; then
   echo "Woochain.org failed to provide signature file, but Woochaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "Woochain.org and Woochaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Woochain Core binary release signing key installed?"
   fi
